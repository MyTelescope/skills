...
- **Never render a pack name for top-ups.** The `pack_id` (e.g. `pack_intro`, `pack_starter`, `pack_growth`, `pack_scale`) is an internal identifier — do NOT show it, title-case it, strip the `pack_` prefix, or otherwise derive a display name from it. Do NOT invent a name like "Intro", "Starter", "Growth", or "Scale" from `pack_id` or from the `description` text. Show each top-up as just: the credit amount, the price, and optionally the short description from the response. No card titles, no column labeled "Pack", no "Buy Intro" / "Buy Starter" buttons — just the numbers. Subscription plans DO have a real `name` field — use that `name` for subscriptions only.
